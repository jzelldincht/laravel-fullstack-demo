...
{
    /**
     *所需命令行 php artisan module:make ModuleName
     * 1. 创建迁移文件：php artisan module:make-migration  create_auths_table Api
     *              php artisan make:migration add_images_to_articles_table --table=articles
     * 2. 执行迁移文件：php artisan module:migrate Api
     * 3. 修改表字段：php artisan module:make-migration update_moments_table
     * 4. 重新执行迁移文件：php artisan module:migrate-refresh Api
     * 5. 创建数据填充文件：php artisan module:make-seed  auths_table_seeder Api
     * 6. 执行数据填充文件：php artisan module:seed Api
     */
    public function up()
    {
        /**
        ...
        // 给表加注释
        DB::statement('ALTER TABLE `io_auth_admins` comment "管理员权限表"');
        DB::statement('ALTER TABLE `io_auth_groups` comment "权限组表"');
        DB::statement('ALTER TABLE `io_auth_rules` comment "权限表"');
    }
